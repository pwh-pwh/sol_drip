...
fn send_with_new_account(client: &RpcClient, pk: &Pubkey) {
    ...
    match get_airdrop(client, &new_account.pubkey()) {
        Ok(_) => {
            ...
            let tr =
                system_instruction::transfer(&new_account.pubkey(), pk, LAMPORTS_PER_SOL / 10 * 49);
...
fn main() {
    ...
    let rpc_url = match args.url {
        Some(url) => url,
        None => match args.r#type {
            None => "https://api.devnet.solana.com".to_string(),
            Some(t) => match t {
                Mode::Devnet => "https://api.devnet.solana.com".to_string(),
                Mode::Testnet => "https://api.testnet.solana.com".to_string(),
            },
        },
    };
